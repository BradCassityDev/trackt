...
const resolvers = {
  ...
  Mutation: {
    uploadPhoto: async (_, { username, photo }) => {
    ...
    updateGoal: async (parent, args, context) => {
      if (context.goal) {
        return await Goal.findByIdAndUpdate(context.goal._id, args, {
          new: true,
        });
      }
    ...
    addMilestone: async (parent, { goalId, milestoneTitle }, context) => {
      if (context.user) {
        const updatedGoal = await Goal.findOneAndUpdate(
          ...
          {
            $push: {
              milestones: { milestoneTitle, username: context.user.username },
            },
          },
    ...
    deleteMilestone: async (parent, { goalId, milestoneId }, context) => {
      if (context.user) {
        const updatedGoal = await Goal.findOneAndUpdate(
          ...
          {
            $pull: {
              milestones: { milestoneId, username: context.user.username },
            },
          },
          { new: true, runValidators: true }
        );
